...
for match in matches:
    try:
        ...
        if match_time < cutoff_time:
            print(f"⏭️ Ignorando partida antiga: {team1} vs {team2} em {event} ({match_time})")
            continue
        ...
        if not any(br.lower() in t for br in BRAZILIAN_TEAMS for t in teams_lower):
            print(f"⏭️ Ignorando partida sem time BR: {team1} vs {team2}")
            continue
